...
class Salt(Task):
    ...
    value of the variable salt.su.master_role (see salt_util.py). This node is
    referred to as the "Salt Master", or the "master node".
    ...
    def __generate_minion_keys(self):
        ...
        for rem in self.remotes.iterkeys():
            ...
            self.su.master_remote.run(args=[
                'sudo',
                'sh',
                '-c',
    ...
    def __set_minion_master(self):
        ...
        master_id = self.su.master_remote.hostname
        for rem in self.remotes.iterkeys():
    ...
    def __provision_minions(self):
        ...
        self.su.start_master()

    def setup(self):
        super(Salt, self).setup()
        log.info("Salt task setup...")
        self.__provision_minions()

    def begin(self):
        log.info("Salt task begin...")
        super(Salt, self).begin()
        self.su.start_minions()
        self.su.check_salt_daemons()
        self.su.ping_minions()

    def end(self):
        super(Salt, self).end()
        log.info("Salt task end...")
        self.su.gather_logs('salt')
    
    def teardown(self):
        super(Salt, self).teardown()
        log.info("Salt task teardown...")
        pass
